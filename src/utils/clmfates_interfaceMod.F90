...
!            patch%itype(pi:pf) = ispval
   ...
   subroutine dynamics_driv(this, nc, bounds_clump,      &
      ...
      do s=1,this%fates(nc)%nsites
         ...
         write(iulog,*) 'dynamics_driv: h2osoi_vol_col: ', waterstatebulk_inst%h2osoi_vol_col(c,1:nlevsoil)
      ...
      !if ( .not.is_restart() ) then
       ...
       do s = 1,this%fates(nc)%nsites
          ...
          do ifp = 1, this%fates(nc)%sites(s)%youngest_patch%patchno
             ...
             write(iulog,*) 'wrap_update_hlmfates_dyn: pre-bc_out: p,esai(p): ', p,esai(p)
             ...
             write(iulog,*) 'wrap_update_hlmfates_dyn: post-bc_out: p,esai(p): ', p,esai(p)
