      ...
      do nc = 1,nclumps
         ...
         do s = 1, this%fates(nc)%nsites
            ...
            do m = natpft_lb,natpft_ub
               ! Make sure that pft_areafrac one-indexed regardless of natpft_lb setting
               ft = m-natpft_lb+1
               this%fates(nc)%bc_in(s)%pft_areafrac(ft)=wt_nat_patch(g,m)

            end do

            !if(abs(sum(this%fates(nc)%bc_in(s)%pft_areafrac(1:natpft_ub-natpft_lb))-1.0_r8).gt.1.0e-9)then
            if(abs(sum(this%fates(nc)%bc_in(s)%pft_areafrac(1:natpft_ub-natpft_lb+1))-1.0_r8).gt.1.0e-9)then
...
!            patch%itype(pi:pf) = ispval
   ...
   subroutine dynamics_driv(this, nc, bounds_clump,      &
      ...
      do s=1,this%fates(nc)%nsites
         ...
         if(use_fates_sp)then
           do ft = 1,natpft_ub-natpft_lb+1 !set of pfts in HLM
